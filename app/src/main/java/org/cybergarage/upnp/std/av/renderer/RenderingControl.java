...
public class RenderingControl implements ActionListener, QueryListener
{
	...
	public boolean actionControlReceived(Action action)
	{
		...
		if (actionName.equals(SETBRIGHTNESS)){
			String brightness = action.getArgumentValue(DESIREDBRIGHTNESS);
			Log.i("brightness",brightness);
			EventBus.getDefault().post(new EventBrightness(brightness));
		}
		if (actionName.equals(GETBRIGHTNESS)){
			int brightness = Settings.System.getInt(DlnaApp.getInstance().getContentResolver(), Settings.System.SCREEN_BRIGHTNESS, 125);
			brightness = (int) (brightness/255f*100);
			Log.i("renderControl",brightness+"");
			action.getArgument(CURRENTBRIGHTNESS).setValue(brightness);
			isActionSuccess = true;
		}
		if (actionName.equals(SETVOLUME)){
			Log.i("renderControl",SETVOLUME+"");
		}
